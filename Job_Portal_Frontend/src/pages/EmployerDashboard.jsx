  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        // Check if it's the user's first time login from the decoded token
        setFirstTimeLogin(decoded.firstTimeLogin === true); // Use firstTimeLogin directly
        setEmail(decoded.email || "User");

        console.log("firstTimeLogin:", decoded.firstTimeLogin); // Debug log to check value
      } catch (err) {
        ...
        <div className="flex-1 p-6">
          {firstTimeLogin ? (
            ...
            <Routes>
              ...
              <Route path="/in-progress-application/:jobId" element={<InProgressApplicationsPage/>}/>
              <Route path="/selected-application/:jobId" element={<SelectedApplications/>}/>
              <Route path="/selectednotselected" element={<SelectedNotSelected/>}/>
            </Routes>
          )}
        </div>
      </div>
