...
const JobList = () => {
  ...
  const [workplaceFilter, setWorkplaceFilter] = useState(""); // Workplace dropdown filter
  const [timeFilter, setTimeFilter] = useState(""); // Time Ago dropdown filter
  ...
  // console.log(timeAgo(postedAt)); // Example: "1d"
  // console.log(timeAgo("2024-05-01"));           // "1y"
  // console.log(timeAgo(new Date().toISOString())); // "0h"
  ...
  // Trigger filters whenever a filter value changes
  useEffect(() => {
    applyFilters();
      ...
      <div className="">
        ...
        <div className="mb-4 flex justify-center flex-wrap">
          <select
            ...
            className="p-2 border border-gray-300 rounded-md mr-4 mt-2"
          >
          ...
          <button
            ...
            className="ml-4 p-2 px-4 bg-gray-200 text-gray-700 rounded-md mt-2 "
          >
            Clear Filters
          </button>
        </div>
