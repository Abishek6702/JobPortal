...
const InProgressApplicationsPage = () => {
    ...
    const fetchApplications = async () => {
      try {
        ...
        if (filteredApplications.length > 0 && filteredApplications[0].jobId) {
          setCompanyInfo({
            logo: filteredApplications[0].jobId.companyLogo,
            name: filteredApplications[0].jobId.companyName,
  ...
  const downloadSelectedResumes = () => {
    selectedApplications.forEach((appId, index) => {
      setTimeout(() => {
        window.open(
          `http://localhost:3000/api/applications/download/${appId}`,
          "_blank"
        );
      }, index * 500);
    });
    ...
    <div className="container mx-auto px-4">
      ...
      >
        ...
        {companyInfo && (
          <>
            <img
              src={`http://localhost:3000/${companyInfo.logo}`}
          ...
          <tbody className="divide-y divide-gray-200">
            {currentApplications.map((app) => (
              <tr key={app._id} className="text-center">
                ...
                <td className="px-6 py-4 whitespace-normal text-left max-w-xs break-words">
                ...
                <td className="px-6 py-4 whitespace-nowrap flex justify-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      window.open(
                        `http://localhost:3000/api/applications/download/${app._id}`,
                        "_blank"
                      )
                    }
